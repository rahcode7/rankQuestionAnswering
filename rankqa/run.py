...
train_filesSQ = [os.path.join('features',
                              'SQuAD-v1.1-train-default-pipeline.preds-part-{}'.format(i)) for i in range(2)]
print(train_filesSQ)
...
test_files = {
    ...
    'SQuAD': [os.path.join('features',
                           'SQuAD-v1.1-dev-default-pipeline.preds-part-{}'.format(i)) for i in range(2)],
...
# subsampling
parser.add_argument('--max-per-q', type=int, default=4, help='Maximum number of training pairs that are sampled'
                                                             ' per question')
...
parser.add_argument('--max-depth', type=int, default=1, help='maximally traverse this deep through candidate answers '
                                                             ' when sampling training pairs')
